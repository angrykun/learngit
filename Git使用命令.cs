Git的使用命令
	...
	3.远程仓库
		3.1添加远程库
			...
			$ git init //在进行本地库与远程库进行关联之前，先进行初始化要上传的目录，否则无法读取到地址进行关联。
			...
			$ git add . //在push到远程库上之前，先在版本库中加入一个文件，否则会出现error: src refspec master does not match any，错误
		...
		3.7多人协作
			...
			3.7.2推送分支
				...
				$ git push origin branch //在master分之下，将本地branch分支推送到远程，如果远程分支没有branch分支，则创建远程分支，如果有远程分支，则将本地分支与远程分支进行关联。
